...
class TimeTableSchedulerZ3 :
    ...
    def __init__(self, module_dict, yesPrint=True) :
    ...
    def optimiseTimetableWithConstraints(self, constraints) :
        self.init_variables()
        self.add_basic_constraints()
        if ("8amLessons" in constraints) :
            print("No 8am Lessons constraint activated")
            No8AMLessonsConstraint(self.allLessons, self.string_to_bool_literal).enforce(self.solver)
        if ("oneFreeDay" in constraints) :
            print("One Free Day constraint activated")
            OneDayFreeConstraint(self.allLessons, self.string_to_bool_literal).enforce(self.solver)
